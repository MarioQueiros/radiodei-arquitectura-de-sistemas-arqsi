...
namespace iRadioDei_WindowsApp
{
    class Music : ActiveRecord
    {
        ...
        public int UpdateMusicInfo(int id, string name, string year)
        {
            try
            {
        ...
        public int InsertMusic(string name, int year)
        {
            try
            {
                int q = ExecuteNonQuery("insert into music (name, launch_year) values ('" + name + "', " + year + ")");
                if (q == -1)
                {
                    return q;
                }
        ...
        public int GetIdMusicByName(string name)
        {
            try
            {
